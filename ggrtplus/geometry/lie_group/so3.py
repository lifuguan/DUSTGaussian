...
from ggrtplus.geometry.lie_group import so3_common as so3c
from ggrtplus.geometry.lie_group.liegroupbase import LieGroupBase
from ggrtplus.geometry.lie_group.utils import isclose
...
class SO3(LieGroupBase):
    ...
    def rotation_angle(self) -> torch.Tensor:
        ...
        rot_err_rad = torch.acos(torch.clamp(0.5 * (trace - 1), min=-1.0+eps, max=1.0-eps))
        # rot_err_rad = acos_safe(torch.clamp(0.5 * (trace - 1), min=-1.0, max=1.0))
        return rot_err_rad
